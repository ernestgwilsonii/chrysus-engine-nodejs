...
chrysusEmitter.on("callTasksSplitter", async chrysusMessage => {
  try {
    ...
    for (let thisTask of tasksArray) {
      ...
      let chrysusTaskId = uuidv4();
      thisTask.meta.chrysusTaskId = chrysusTaskId;
      thisTask.meta.chrysusStatus = "TASKSPLITTER";
      console.log("ChrysusEngine thisTask:\n" + JSON.stringify(thisTask, null, 2));
      chrysusEmitter.emit("callTaskLauncher", thisTask); // Send this task to the Chrysus Task Launcher
    }
  } catch (error) {
    console.error(error);
  }
...
chrysusEmitter.on("callTaskLauncher", async chrysusTask => {
  try {
    chrysusTask.meta.chrysusStatus = "TASKLAUNCHER";
    ...
    chrysusEmitter.emit("callOutputProcessor", taskResults); // Send the results to the Chrysus Output Processor
  } catch (error) {
    console.error(error);
  }
});
...
chrysusEmitter.on("callOutputProcessor", async chrysusTaskResults => {
  try {
    chrysusTaskResults.meta.chrysusStatus = "OUTPUTPROCESSOR";
    await sleep(2000); // Simulate some processing delay!
